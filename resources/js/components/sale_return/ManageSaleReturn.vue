<template>
  <div class="">
      ...
      <div class="col-lg-12 mb-4">
        ...
        <div class="card">
          <div class="card-header py-3">
            <h3 class="m-0 font-weight-bold" title="">Sale Return List</h3>
            <p class="text-secondary m-0">Stores information about each sale return</p>
          </div>
          <div class="card-body p-0 m-0">
            <div class="row p-2">
              <div class="input-group">
                ...
                <input
                  type="text"
                  class="form-control"
                  placeholder="Search by sale return date, customer name. (Type and Enter)"
                  v-model="search"
                />
            ...
            <div class="table-responsive">
              ...
              >
                <thead class="thead-light">
                  <tr>
                    ...
                    <th scope="col" class="text-nowrap">
                      <a href="#" @click.prevent="changeShort('sale_return_date')">
                        Return Date</a
                      >
                      <span
                        v-if="
                  ...
                  <tr>
                    ...
                    <th colspan="1" class="text-nowarp px-1" style="min-width: 200px">
                      <input
                        ...
                        class="form-control-sm"
                        style="width: 100%"
                        v-model="params.sale_return_date"
                      />
                    </th>
                ...
                <tbody v-if="sale_returns && paginator.totalRecords > 0">
                  <tr v-for="sale_return in sale_returns.data" :key="sale_return.id">
                    <!-- <td class="text-center">
                      <input
                        type="checkbox"
                        :value="sale_return.id"
                        v-model="checked"
                    ...
                    <td class="text-nowrap">
                      <a
                        @click="openModal(sale_return.id)"
                        href="#"
                        data-toggle="modal"
                        data-target="#recordModal"
                        >{{ sale_return.sale_return_date }}</a
                      >
                    </td>
                    <td class="text-nowrap">{{ sale_return.customer.customer_name }}</td>
                    <td class="text-nowrap text-right">{{ sale_return.total_amount }}</td>
                    <td class="text-nowrap text-center">{{ sale_return.discount_percentage }}</td>
                    <td class="text-nowrap text-right">{{ sale_return.discount_amount }}</td>
                    <td class="text-nowrap text-center">{{ sale_return.vat_percentage }}</td>
                    <td class="text-nowrap text-right">{{ sale_return.vat_amount }}</td>
                    <td class="text-nowrap text-right">{{ sale_return.net_amount }}</td>

                    <td class="text-right text-nowrap">
                      <div class="btn-group" option="group">
                        <!-- <button @click="exportInvoicePdf(sale_return.id)" :class="'btn my-btn-danger export-invoice-btn-pdf-'+sale_return.id">
                  Export Invoice
                </button> -->
                        <button
                          @click="openModal(sale_return.id)"
                        ...
                        <!-- <router-link
                          :to="`/sale-returns/${sale_return.id}`"
                          class="btn btn-sm my-btn-primary"
                          ><i class="fa fa-eye"></i> View</router-link
                        > -->
                        <router-link
                          :to="`/sale-returns/${sale_return.id}/edit`"
                          class="btn btn-sm btn-primary px-2 mx-1"
                          ><i class="fa fa-edit"></i> Edit</router-link
                        >
                        <a
                          @click="deletesale(sale_return.id)"
    ...
    <!--Row-->
    <!-- Bootstrap Modal -->
    <ViewSaleReturn :record="record" :fileExtension="fileExtension" />
    <!-- Modal End -->
  </div>
</template>
...
import ViewSaleReturn from "./ViewSaleReturn.vue";
export default {
  name: "sale",
  components: { ViewSaleReturn },
  data() {
    return {
      fileExtention: null,
      record: {},
    ...
    async getSales(page = 1) {
      ...
      await axios
        ...
        .then((response) => {
          ...
          this.sale_returns = response.data;
