<template>
        ...
        <div class="card mt-3 py-1">
          <div class="card-header py-1">
            <h3 class="text-center fw-bold">
              <i class="fa fa-chart-pie"></i> Stock Report
            </h3>
          </div>
          <form id="form" @submit.prevent="submitForm">
            <div class="card-body py-1">
              ...
              <div class="form-group py-0 my-0">
                <label for=""
                  >Stock Status <span class="my-text-danger fw-bold"> *</span></label
                >
                <select
                  name=""
                  id=""
                  ...
                  :class="{ 'is-invalid': form.errors.has('stock_status') }"
                >
                  <!-- <option value="">--select stock status--</option> -->
                  <option value="all" selected>All</option>
                  <option value="stock_available" >Stock Avaliable</option>
                  <option value="stock_out" >Stock Out</option>
                  <!-- <option value="stock_alert" selected>Stock Alert</option> -->
                </select>
                <HasError :form="form" field="stock_status" />
              </div>
      ...
      <div class="row" v-if="stocks.length > 0">
        <div id="printableContent" class="col-md-10 offset-md-1 border my-3 rounded-sm">
          ...
          <table class="table table-sm">
            ...
            <tr>
              <td
                class="fw-bold text-left" colspan="4"
              >
                Stock Total
              </td>
  ...
  export default {
    ...
    methods: {
          ...
          .then((response) => {
            if (response.data.report_type == "view") {
              ...
              if (this.form.btn_type == "excel") {
                ...
                fileLink.setAttribute("download", "stock-report.xlsx");
                ...
                var fileLink = document.createElement("a");
                fileLink.href = fileURL;
                fileLink.setAttribute("download", "stock-report.pdf");
                document.body.appendChild(fileLink);
                fileLink.click();
              } else if (this.form.btn_type == "print") {
      ...
      calculateStockTotal() {
        return this.stocks.reduce((total, stock) => total + Number(stock.stock_quantity), 0);
      },
