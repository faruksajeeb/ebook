<template>
  <div>
    <div class="row">
      <router-link to="/manage-product" class="btn btn-primary">All Product </router-link>
    </div>
...
export default {
  ...
  created() {
    ...
    // Category Collected
    axios.get("/api/manage-category/").then(({ data }) => (this.categories = data));

    // Supplier Collected
    axios.get("/api/manage-supplier/").then(({ data }) => (this.suppliers = data));
  },
  ...
  methods: {
    ...
    ProductUpdate() {
      let id = this.$route.params.id;
      axios
        .patch("/api/manage-product/" + id, this.form)
        .then(() => {
